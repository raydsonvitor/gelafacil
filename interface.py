...
class TrueBuyInterface:
    def __init__(self, root, tela_width, tela_height, version):
        ...
        self.yon = False
        self.tp_cdm = self.tp_idv_1 = self.tp_idv_2 = self.tp_gdm = self.tp_gdm_tp_editar_mercadoria = self.tp_5 = self.tp_6 = self.tp_7 = self.tp_gdc = self.tp_9 = self.tp_gdv = None
        ...
        self.clock.place(relx=0.93, rely=0.915)
        self.clock.lift()
        self.update_clock()
        ...
        if self.previsao_fat_dia:
            self.previsao_fat_dia = f'{(helper.format_to_moeda(self.previsao_fat_dia))}'
        self.previsao_fat_mes = database.get_previsao_faturamento_mes()
        if self.previsao_fat_mes:
            self.previsao_fat_mes = f'{(helper.format_to_moeda(self.previsao_fat_mes))}'
    ...
    def abrir_tp_password(self, janela):
        ...
        self.tp_password_entry = CTkEntry(self.tp_password, width=400, height=50, font=self.tp_cdm_fonte_padrao_bold)
    ...
    def abrir_idv_interface(self):
        #Header
        self.tp_idv_label_title = CTkLabel(self.root, text=f'  GELA FACIL {self.version}  ', font=CTkFont('helvetica', 80, 'bold'), compound='right', 
        ...
        self.tp_idv_entry_codbar = CTkEntry(self.root, width=700, height=35, border_color='black')
        ...
        self.tp_idv_frame_footer_label_0 = CTkLabel(self.tp_idv_frame_footer, text='F1 - Cadastrar Mercadoria     F2 - Remover item da compra     F3 - Finalizar Compra     F4 - Buscar Mercadoria     F5 - Gerenciamento de Clientes     F6 - Fiar Compra     F7 - Buscar Fiação        F11 - Gerenciamento de Vendas      F12 - Gerenciar Caixa', 
        ...
        self.tp_idv_entry_codbar.bind('<Return>', lambda event: self.check_which_treeview_to_use_return())
    ...
    def check_which_treeview_to_use_return(self):
    ...
    def inserir_item_na_compra(self, feedback, quantidade=1):
        try:
            ...
            self.tp_idv_entry_codbar.delete(0, END)
            self.root.after(100, self.tp_idv_treeview.yview_moveto(1.0))
    ...
    def remove_item_selecionado(self):
        ...
        if selected_item:
            ...
            if yon_0:
                ...
                item_removido = self.carrinho.pop(int(item_values[0])-1)
                #remove da treeview
    ...
    def redefinir_item_indexs_da_compra(self):
        for index, child in enumerate(self.tp_idv_treeview.get_children(), start=1):
            #obtendo o item e removendo seu index antigo
            item_sem_index = self.tp_idv_treeview.item(child, 'values')[1:]
            #formatando o index para tupla
            index_tupla = tuple(str(index))
            #adicionar index novo ao item
            item = index_tupla + item_sem_index
            #remove o item referente ao index da instanciado
            self.tp_idv_treeview.delete(child)
            #recoloca o item novamente com novo id
            self.tp_idv_treeview.insert('',index, values=item)     
    ...
    def validate_tp_idv_1(self):  # forms pgmt
        ...
        try:
            ...
            if yon_0:
                ...
                self.total_restante = helper.format_to_float(self.total_restante) - pago_abatido
        ...
        try:
            ...
            if self.customer_id:
                ...
                if self.tp_password_feedback:
                    cfm = database.delete_oncredits_by_customer_id_and_insert_sale_into_tables(self.customer_id, items, self.payments, self.troco, self.desconto)
                    if cfm:
    ...
    def reset_root(self, event=None):
        self.tp_idv_label_title.configure(text=f'  GELA FACIL {self.version}  ')
        self.tp_idv_frame_status_label.configure(text='Aguardando Código de barras...')
	...
	#widgets
        ...
        self.tp_cdm_label_categoria.place(relx=0.02, rely=0.48)
        ...
        self.tp_cdm_combobox_categoria.place(relx=0.13, rely=0.46)
        self.tp_cdm_combobox_categoria.set('')
        self.tp_cdm_combobox_categoria_sinalizer = CTkLabel(self.tp_cdm ,text_color='red')

        self.tp_cdm_label_marca = CTkLabel(self.tp_cdm, text='Marca:', font=self.tp_cdm_fonte_padrao_bold)
        self.tp_cdm_label_marca.place(relx=0.02, rely=0.61)
        self.tp_cdm_entry_marca = CTkEntry(self.tp_cdm, font=self.tp_cdm_fonte_padrao_bold, width=400, height=50)
        self.tp_cdm_entry_marca.place(relx=0.13, rely=0.59)
        self.tp_cdm_entry_marca_sinalizer = CTkLabel(self.tp_cdm ,text_color='red')

        #COLUMN 2

        self.tp_cdm_label_precocompra = CTkLabel(self.tp_cdm, text='Preço Compra:', font=self.tp_cdm_fonte_padrao_bold)
        self.tp_cdm_label_precocompra.place(relx=0.52, rely=0.22)
        self.tp_cdm_entry_precocompra = CTkEntry(self.tp_cdm, font=self.tp_cdm_fonte_padrao_bold, width=400, height=50)
        self.tp_cdm_entry_precocompra.place(relx=0.64, rely=0.20)
        ...
        self.tp_cdm_label_precovenda.place(relx=0.52, rely=0.35)
        self.tp_cdm_entry_precovenda = CTkEntry(self.tp_cdm, font=self.tp_cdm_fonte_padrao_bold, width=400, height=50)
        self.tp_cdm_entry_precovenda.place(relx=0.64, rely=0.33)
        self.tp_cdm_entry_precovenda_sinalizer = CTkLabel(self.tp_cdm ,text_color='red')

        self.tp_cdm_label_estoque = CTkLabel(self.tp_cdm, text='Estoque:', font=self.tp_cdm_fonte_padrao_bold)
        self.tp_cdm_label_estoque.place(relx=0.52, rely=0.48)
        self.tp_cdm_entry_estoque = CTkEntry(self.tp_cdm, font=self.tp_cdm_fonte_padrao_bold, width=400, height=50)
        self.tp_cdm_entry_estoque.place(relx=0.64, rely=0.46)
        self.tp_cdm_entry_estoque_sinalizer = CTkLabel(self.tp_cdm ,text_color='red')

        self.tp_cdm_label_fornecedor = CTkLabel(self.tp_cdm, text='Fornecedor:', font=self.tp_cdm_fonte_padrao_bold)
        self.tp_cdm_label_fornecedor.place(relx=0.52, rely=0.61)
        self.tp_cdm_entry_fornecedor = CTkEntry(self.tp_cdm, font=self.tp_cdm_fonte_padrao_bold, width=400, height=50)
        self.tp_cdm_entry_fornecedor.place(relx=0.64, rely=0.59)
        self.tp_cdm_entry_fornecedor_sinalizer = CTkLabel(self.tp_cdm ,text_color='red')

        self.tp_cdm_label_status = CTkLabel(self.tp_cdm, text='status:', font=self.tp_cdm_fonte_padrao_bold)
        self.tp_cdm_label_status.place(relx=0.52, rely=0.74)
        self.tp_cdm_combobox_status = CTkComboBox(self.tp_cdm, values= ('Ativo', 'Não Ativo'), font=self.tp_cdm_fonte_padrao, width=400, height=50)
        self.tp_cdm_combobox_status.place(relx=0.64, rely=0.72)
        self.tp_cdm_combobox_status.set('')
        self.tp_cdm_combobox_status_sinalizer = CTkLabel(self.tp_cdm ,text_color='red')
        ...
        self.tp_cdm.after(100, self.tp_cdm_entry_codbar.focus_set)
    ...
    def validar_dados_do_registro_de_mercadorias(self):
        ...
        try:
            ...
            if not codbar_inserido:
                ...
                self.tp_cdm_entry_codbar_sinalizer.place(relx=0.13, rely=0.28)
            ...
            if not desc_inserida:
                ...
                self.tp_cdm_combobox_descricao_sinalizer.place(relx=0.13, rely=0.41)
            ...
            if categoria_inserida not in self.mercadoria_categorias:
                ...
                self.tp_cdm_combobox_categoria_sinalizer.place(relx=0.13, rely=0.54)
                self.tp_cdm_combobox_categoria.focus_set()
                return
            marca_inserida = self.tp_cdm_entry_marca.get().strip()
            if not marca_inserida or marca_inserida.isdigit():
                self.tp_cdm_entry_marca_sinalizer.configure(text='O nome da marca deve conter letras.')
                self.tp_cdm_entry_marca_sinalizer.place(relx=0.13, rely=0.67)
                self.tp_cdm_entry_marca.focus_set()
                return
            ...
            if not precocompra_inserido:
                ...
                self.tp_cdm_entry_precocompra_sinalizer.place(relx=0.70, rely=0.28)
            ...
            if not precovenda_inserido:
                ...
                self.tp_cdm_entry_precovenda_sinalizer.place(relx=0.70, rely=0.41)
            ...
            if precovenda_inserido:            
                try:
                    ...
                    self.tp_cdm_entry_precovenda.focus_set()
                    return
            estoque_inserido = self.tp_cdm_entry_estoque.get().strip()
            try:
                estoque_inserido = int(estoque_inserido)
                if estoque_inserido < 0:
                    raise ValueError
            except Exception as e:
                self.tp_cdm_entry_estoque_sinalizer.configure(text='A quantidade deve ser um numero inteiro.')
                self.tp_cdm_entry_estoque_sinalizer.place(relx=0.70, rely=0.54)
                self.tp_cdm_entry_estoque.focus_set()
                return
            fornecedor_inserido = self.tp_cdm_entry_fornecedor.get().strip()
            status_inserido = self.tp_cdm_combobox_status.get().strip()
            if status_inserido not in ('Ativo', 'Não Ativo'):
                self.tp_cdm_combobox_status_sinalizer.configure(text='Selecione um status válido.')
                self.tp_cdm_combobox_status_sinalizer.place(relx=0.70, rely=0.82)
                self.tp_cdm_combobox_status.focus_set()
                return
            print('mercadoria aprovado para o database')
            row = (codbar_inserido, desc_inserida,  categoria_inserida ,marca_inserida,precocompra_inserido, precovenda_inserido ,estoque_inserido, fornecedor_inserido, status_inserido, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def tp_cdm_clear_sinalizers(self):
        ...
        self.tp_cdm_entry_marca_sinalizer.place_forget()
        self.tp_cdm_entry_precocompra_sinalizer.place_forget()
        self.tp_cdm_entry_precovenda_sinalizer.place_forget()
        self.tp_cdm_entry_estoque_sinalizer.place_forget()
        self.tp_cdm_entry_fornecedor_sinalizer.place_forget()
    ...
    def tp_cdm_widgets_clear(self):
        ...
        self.tp_cdm_entry_estoque.delete(0, END)
        self.tp_cdm_entry_fornecedor.delete(0, END)
        self.tp_cdm_entry_marca.delete(0, END)
    ...
    def abrir_gdm(self):
        if not self.tp_gdm:
            ...
            self.tp_gdm.after(100, self.search_entry.focus_set)
    ...
    def abrir_tp_gdm_tp_editar_mercadoria(self, event=None):
        ...
        self.tp_gdm_tp_editar_mercadoria_entry_descricao.insert(0, self.item_values[2])
        ...
        self.tp_gdm_tp_editar_mercadoria_combobox_categoria.set(self.item_values[3])


        self.tp_gdm_tp_editar_mercadoria_label_marca = CTkLabel(self.tp_gdm_tp_editar_mercadoria, text='Marca:', font=self.tp_gdm_fonte_padrao_bold)
        self.tp_gdm_tp_editar_mercadoria_label_marca.place(relx=0.02, rely=0.61)
        self.tp_gdm_tp_editar_mercadoria_entry_marca = CTkEntry(self.tp_gdm_tp_editar_mercadoria, font=self.tp_gdm_fonte_padrao, width=400, height=50)
        self.tp_gdm_tp_editar_mercadoria_entry_marca.place(relx=0.13, rely=0.59)
        self.tp_gdm_tp_editar_mercadoria_entry_marca_sinalizer = CTkLabel(self.tp_gdm_tp_editar_mercadoria ,text_color='red')
        self.tp_gdm_tp_editar_mercadoria_entry_marca.insert(0, self.item_values[4])
        ...
        self.tp_gdm_tp_editar_mercadoria_entry_precocompra.insert(0, self.item_values[5])
        ...
        self.tp_gdm_tp_editar_mercadoria_entry_precovenda.insert(0, self.item_values[6])
        ...
        self.tp_gdm_tp_editar_mercadoria_entry_estoque.insert(0, self.item_values[7])
        ...
        self.tp_gdm_tp_editar_mercadoria_label_fornecedor.place(relx=0.52, rely=0.61)
        self.tp_gdm_tp_editar_mercadoria_entry_fornecedor = CTkEntry(self.tp_gdm_tp_editar_mercadoria, font=self.tp_gdm_fonte_padrao, width=400, height=50)
        self.tp_gdm_tp_editar_mercadoria_entry_fornecedor.place(relx=0.64, rely=0.59)
        self.tp_gdm_tp_editar_mercadoria_entry_fornecedor_sinalizer = CTkLabel(self.tp_gdm_tp_editar_mercadoria ,text_color='red')
        self.tp_gdm_tp_editar_mercadoria_entry_fornecedor.insert(0, self.item_values[8])
 
        self.tp_gdm_tp_editar_mercadoria_label_status = CTkLabel(self.tp_gdm_tp_editar_mercadoria, text='status:', font=self.tp_gdm_fonte_padrao_bold)
        self.tp_gdm_tp_editar_mercadoria_label_status.place(relx=0.52, rely=0.74)
        self.tp_gdm_tp_editar_mercadoria_combobox_status = CTkComboBox(self.tp_gdm_tp_editar_mercadoria, values= ('Ativo', 'Não Ativo'), font=self.tp_gdm_fonte_padrao, width=400, height=50)
        self.tp_gdm_tp_editar_mercadoria_combobox_status.place(relx=0.64, rely=0.72)
        self.tp_gdm_tp_editar_mercadoria_combobox_status_sinalizer = CTkLabel(self.tp_gdm_tp_editar_mercadoria ,text_color='red')
        self.tp_gdm_tp_editar_mercadoria_combobox_status.set(self.item_values[9])
    ...
    def validate_tp_gdm_tp_editar_mercadoria(self, event=None):
        ...
        try:
            ...
            if categoria_inserida not in self.mercadoria_categorias:
                ...
                return
            marca_inserida = self.tp_gdm_tp_editar_mercadoria_entry_marca.get().strip()
            if not marca_inserida or marca_inserida.isdigit():
                self.tp_gdm_tp_editar_mercadoria_entry_marca_sinalizer.configure(text='O nome da marca deve conter letras.')
                self.tp_gdm_tp_editar_mercadoria_entry_marca_sinalizer.place(relx=0.13, rely=0.67)
                self.tp_gdm_tp_editar_mercadoria_entry_marca.focus_set()
                return
            ...
            try:
                ...
                return
            fornecedor_inserido = self.tp_gdm_tp_editar_mercadoria_entry_fornecedor.get().strip()
            status_inserido = self.tp_gdm_tp_editar_mercadoria_combobox_status.get().strip()
            if status_inserido not in ('Ativo', 'Não Ativo'):
                self.tp_gdm_tp_editar_mercadoria_combobox_status_sinalizer.configure(text='Selecione um status válido.')
                self.tp_gdm_tp_editar_mercadoria_combobox_status_sinalizer.place(relx=0.70, rely=0.82)
                self.tp_gdm_tp_editar_mercadoria_combobox_status.focus_set()
                return
            cfm = self.get_yes_or_not(self.tp_gdm_tp_editar_mercadoria)
            if cfm:
                fb = database.update_product((codbar_inserido, desc_inserida, categoria_inserida , marca_inserida, precocompra_inserido, precovenda_inserido, estoque_inserido, fornecedor_inserido, status_inserido, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), self.item_values[0]))
                if fb:
    ...
    def tp_gdm_tp_editar_mercadoria_clear_sinalizers(self):
        ...
        self.tp_gdm_tp_editar_mercadoria_entry_marca_sinalizer.place_forget()
        ...
        self.tp_gdm_tp_editar_mercadoria_entry_fornecedor_sinalizer.place_forget()
    ...
    def abrir_tp_6(self):
        ...
        self.clientes = database.get_clientes()
        if self.clientes:
            self.clientes = [item[0] for item in self.clientes]
        else:
            self.clientes = ('Nenhum Cliente encontrado')

        self.tp_6_titlo = CTkLabel(self.tp_6, text='Lançar na conta do Cliente', font=CTkFont('arial', 35, 'bold'))
        ...
        self.tp_6_button_1 = CTkButton(self.tp_6, text='Fiar Compra', font=self.tp_cdm_fonte_padrao_bold, height = 50, command=lambda:self.validate_tp_6(), text_color='black', fg_color='green', hover_color='green')
        self.tp_6_button_1.place(relx=0.5, rely=0.6, anchor='n')
    ...
    def validate_tp_6(self):
        ...
        try:
            ...
            if self.cliente_selecionado not in self.clientes:
                ...
                return
            cliente_id = database.get_cliente_id_by_name(self.cliente_selecionado)[0]
            if cliente_id:
    ...
    def finalizar_fiacao(self, customer_id):
        items = json.dumps(self.carrinho)
        data_hoje = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cfm = database.insert_into_oncredit(customer_id, items, data_hoje)
        if cfm:
            cfm_1 = CTkMessagebox(self.tp_6, message=f"Fiação realizada para o cliente: {self.cliente_selecionado.upper()}!", icon='check', title='Sucesso')
    ...
    def abrir_tp_7(self):
        if self.tp_7:
            return
        if (self.get_treeview_itens_number()) > 0:
            msg = CTkMessagebox(self.root, message=f'Finalize ou cancele a compra atual antes de realizar esta operação.', icon='warning', title='Atenção')
            self.root.wait_window(msg)
            self.tp_idv_entry_codbar.focus_set()
            return
        ...
        self.tp_7_titlo = CTkLabel(self.tp_7, text='Buscar Conta do Cliente', font=CTkFont('arial', 35, 'bold'))
        ...
        self.tp_7_button_1 = CTkButton(self.tp_7, text='Buscar Dados', font=self.tp_cdm_fonte_padrao_bold, height = 50, command=lambda:self.validate_tp_7(), fg_color='green', hover_color='green')
    ...
    def validate_tp_7(self):
        ...
        try:
            ...
            if self.cliente_selecionado not in self.clientes:
                ...
                return
            cliente_id = database.get_customer_id_by_name(self.cliente_selecionado)[0]
            if cliente_id:
                self.customer_id = int(cliente_id)
                yon = self.get_yes_or_not(self.tp_7, f'Buscar todos os dados da conta do cliente {self.cliente_selecionado.upper()}?')
                self.data = database.get_all_data_from_customer_by_id(cliente_id)
                if yon:    
    ...
    def get_all_data_to_treeview(self, data):
        try:
            if data:
                for item in data:
                    self.inserir_item_na_compra(('', '', item[3], '', '', '', item[4], '', '', '', '', ''), item[5])
                self.fechar_tp_7()
    ...
    def imprimir_conta_cliente(self, customer_id, customer_name):
        try:
            return
            data = database.get_all_data_from_customer_by_id(customer_id)
            print(data)
            nota = []
    ...
    def get_widget_data(self):
        try:
            ...
            with open('txts/fechamentodecaixa_lastdatetime.txt', 'r') as a:
                date_str = a.readline()
    ...
    def tp_gdc_validate_suprimento(self):
        ...
        try:
            ...
            #validando valor
            valor_inserido = self.tp_gdc_entry_2.get()
            if valor_inserido:
                try:
                    float(valor_inserido)
                except:
    ...
    def fechar_caixa(self, valor_inserido):
        try:
            ...
            with open('txts/fechamentodecaixa_lastdatetime.txt', 'w') as a:
    ...
    def tp_gdc_1_excluir_item(self):
        try:
            if 'Venda' in self.item_values[1]:
                item_id = self.item_values[0]
                yon = self.get_yes_or_not(self.tp_gdc, 'Confirmar a exclusão da venda?')
                if not yon:
                    return
                cfm = database.delete_sale_by_id(item_id)
                if cfm:
                    msg = CTkMessagebox(message=f"Item excluido com sucesso!", icon='check', title='Sucesso')
                    self.tp_gdc_1.wait_window(msg)
                    self.fechar_tp_gdc_1()
                else:
                    raise Exception('Erro na hora de excluir o item.')
            elif 'Sangria' in self.item_values[1]:
                item_id = self.item_values[0]
                yon = self.get_yes_or_not(self.tp_gdc, 'Confirmar a exclusão da sangria de caixa?')
                if not yon:
                    return
                cfm = database.delete_sangria_by_id(item_id)
                if cfm:
                    msg = CTkMessagebox(message=f"Item excluido com sucesso!", icon='check', title='Sucesso')
                    self.tp_gdc_1.wait_window(msg)
                    self.fechar_tp_gdc_1()
                    ...
                    raise Exception('Erro na hora de excluir o item.')
            elif 'Suprimento' in self.item_values[1]:
                item_id = self.item_values[0]
                yon = self.get_yes_or_not(self.tp_gdc, 'Confirmar a exclusão do suprimento de caixa?')
                if not yon:
                    return
                cfm = database.delete_suprimento_by_id(item_id)
                if cfm:
                    msg = CTkMessagebox(message=f"Item excluido com sucesso!", icon='check', title='Sucesso')
                    self.tp_gdc_1.wait_window(msg)
                    self.fechar_tp_gdc_1()
            ...
            CTkMessagebox(self.tp_gdc_1, message=f'Erro: {e}', icon='cancel', title='Erro')
            return
    ...
    def abrir_tp_gdv(self):
        ...
        self.tp_gdv_previsao_dia_label_1 = CTkLabel(self.tp_gdv, text=self.previsao_fat_dia, font=CTkFont('arial', 35, 'bold'), text_color='orange', image=previsao_img, compound='left')
        ...
        self.tp_gdv_previsao_mes_label_1 = CTkLabel(self.tp_gdv, text=self.previsao_fat_mes, font=CTkFont('arial', 35, 'bold'), text_color='orange', image=previsao_img, compound='left')
    ...
    def get_tp_gdv_widget_data(self):
        try:
            ...
            if 0 <= hora < 6: #se for madrugada (o dia base é o anterior)
                base_day = datetime.strptime(date, '%Y-%m-%d') - timedelta(days=1)
                base_day = datetime.strftime(base_day, '%Y-%m-%d')
            else:
                base_day = date
    ...
    def update_tp_gdv(self):
        ...
        self.tp_gdv_faturamento_dia_label_1.configure(text=f'{helper.format_to_moeda(dados[1], 'R$')}')
        self.tp_gdv_faturamento_mensal_label_1.configure(text=f'{helper.format_to_moeda(dados[0], 'R$')}')
    ...
    def abrir_clientes_registro(self, edit, event=None):
        ...
        self.tp_clientes_registro.after(100, self.tp_clientes_registro_label_nome.focus_set())
    ...
    def tp_clientes_registro_registrar_cliente(self):
        ...
        row = (nome, cpf, whatsapp, email,  sexo,  endereco, data_nascimento, limite, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def tp_clientes_registro_editar_cliente(self, cliente_id):
        ...
        row = (nome, cpf, whatsapp, email,  sexo, endereco, data_nascimento, limite, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), datetime.now().strftime('%Y-%m-%d %H:%M:%S'), cliente_id)
    ...
    def buscar_conta_cliente(self, event=None):
        try:
            selected_item = self.clientes_treeview.focus()
            self.item_values = self.clientes_treeview.item(selected_item, "values")
            on_credit_data = database.get_all_data_from_customer_by_id(self.item_values[0])
            if not on_credit_data:
                msg = CTkMessagebox(self.root, message=f'Nenhum dado encontrado para o(a) cliente: {self.item_values[1]}', icon='warning', title='Atenção')
                return
            for i, item in enumerate(on_credit_data): 
                conjunto_compras = json.loads(item[2])
                item_len = 0
                for item_da_compra in conjunto_compras:
                    produto = tuple(item_da_compra[:-1])
                    quantidade = item_da_compra[-1]
                    print(produto, quantidade)
                    self.inserir_item_na_compra(produto, quantidade)
                    item_len = 1
                if item_len:
                    self.set_idv_conta_cliente_modo(self.item_values[1], self.item_values[0])
                    self.fechar_tp_clientes()
        except Exception as e:
            print(e)
            self.reset_root()

    def set_idv_conta_cliente_modo(self, cliente_nome, cliente_id):
        if len(cliente_nome) > 18:
            cliente_nome = cliente_nome[:17]
       
        #modificando visualmente e funcionalmente o idv
        self.tp_idv_label_title.configure(text=f'Conta: {cliente_nome}')
        self.root.unbind('<F2>')#, lambda event: self.remover_item_da_compra())
        #informando ao sistema o id do cliente para o momento de finalização da compra (remover as pendecias da tabela on_credit)
